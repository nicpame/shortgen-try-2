...
# Define the target file name as a constant
TRANSCRIPTION_FILE_NAME = "raw_transcription.json"
VIDEOS_DIR = "./data/dled_shorts"



def scan_directory(relative_path):
    """ Scans only the direct subfolders inside the given directory for the target file """
    abs_path = os.path.abspath(relative_path)  # Convert relative path to absolute
    ...
    for subfolder in os.listdir(abs_path):  # List items in the directory
        ...
        if os.path.isdir(subfolder_path):  # Check if it's a directory
            target_file_path = os.path.join(subfolder_path, TRANSCRIPTION_FILE_NAME)
            
            if os.path.isfile(target_file_path):  # Check if the file exists
                normalize_transcription(target_file_path)  # Pass file path to processing function




def normalize_transcription(json_file_path):
    """
    ...
    output_path = os.path.join(os.path.dirname(json_file_path), "normalized_transcription.json")
